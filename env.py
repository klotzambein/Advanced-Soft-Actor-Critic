...
imi_learning_reward_columns = ['pelvis_ty', 'pelvis_tilt', 'pelvis_list', 'ankle_angle_l', 'ankle_angle_r',
...
class OpenSimEnv(Env):
        ...
        for root, dirs, files in os.walk(data_dir):
            ...
            for file in files:
                if file.endswith(".csv"):
                    ...
                    # start_times = [df['time'][t] for t in np.random.randint(0, len(
                    #     df) - 100, (100, )) if df["knee_angle_l"][t] > -0.5 and df["time"][t] > 6.75]
                    # print(start_times, "start_times")
                    # Start when already walking
                    start_times = [6.75, 7.0, 7.25, 7.5]
                    self.imitation_data_start_times[name] = start_times

                    self.imitation_data_tensor[name] = df.loc[:,
                                                              imi_learning_reward_columns].values

        self.env = RUGTFPEnv(model_name=model_name, visualize=visualize,
    ...
    def reset(self):
        # print("Reset")
        # random.choice(list(self.imitation_data.keys()))
        self.current_imitation_data = "new"
        imi = self.imitation_data[self.current_imitation_data]

        t = random.choice(
            self.imitation_data_start_times[self.current_imitation_data])
        self.start_time = t
        self.t = t
        ...
        pose.compute_velocities(
            pose_next, bounds_next['time'] - bounds['time'])
